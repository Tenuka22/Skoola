  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium">
            Manage your educational institution's human capital.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4">
        <p className="text-sm font-medium text-muted-foreground">
          ...
          <span className="text-foreground">
            {staffMembers.length > 0 ? (page - 1) * limit + 1 : 0}
          </span>{' '}
          to{' '}
          <span className="text-foreground">
            {Math.min(page * limit, totalEmployees)}
          </span>{' '}
          of <span className="text-foreground">{totalEmployees}</span> entries
        </p>
      ...
      <StaffModals
        ...
        onDeleteConfirm={(id) =>
          deleteMutation.mutate({ path: { staff_id: id } })
        }
        isAddOpen={isAddOpen}
