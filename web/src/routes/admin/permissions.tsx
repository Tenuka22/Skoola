...
function PermissionsPage() {
  ...
  const { data: permissionSetsData, isLoading: isLoadingPermissionSets, refetch: refetchPermissionSets } = useQuery({
      ...
      <Tabs defaultValue="permissions" className="space-y-6">
          ...
          <Card className="border-none shadow-xl overflow-hidden">
            <CardHeader className="flex flex-col space-y-4 border-b bg-muted/20 px-6 py-5 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
              <div className="space-y-1">
                <CardTitle className="text-2xl font-black">Global Permissions</CardTitle>
            ...
            <CardContent className="p-0">
              <PermissionsTable
                ...
                setSorting={(updater) => {
                  const sortingState = typeof updater === 'function' ? updater([]) : updater
                  if (sortingState.length > 0) {
                    setPermissionsSortBy(sortingState[0].id)
                    setPermissionsSortOrder(sortingState[0].desc ? 'desc' : 'asc')
                  } else {
        ...
        <TabsContent value="permission-sets" className="space-y-4">
          <Card className="border-none shadow-xl overflow-hidden">
            <CardHeader className="flex flex-col space-y-4 border-b bg-muted/20 px-6 py-5 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
              <div className="space-y-1">
                <CardTitle className="text-2xl font-black">Permission Sets</CardTitle>
                <CardDescription className="font-medium">
