...
function StudentsPage() {
  ...
  const [studentStatusFilter, setStudentStatusFilter] = React.useState<string>('all')
  
  const [studentToDelete, setStudentToDelete] = React.useState<StudentResponse | null>(null)
  const [studentToEdit, setStudentToEdit] = React.useState<StudentResponse | null>(null)
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        <p className="text-red-500 mb-4">Error loading students: {error.message}</p>
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight">{totalStudents} Students</h1>
          <p className="text-muted-foreground font-medium">Manage your educational institution's student records.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <HugeiconsIcon icon={PlusSignIcon} className="w-4 h-4 mr-2" /> Add Student
          </Button>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-xl p-2">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuRadioGroup value={studentStatusFilter} onValueChange={setStudentStatusFilter}>
                ...
                <DropdownMenuRadioItem value="Withdrawn">Withdrawn</DropdownMenuRadioItem>
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4">
        <p className="text-sm font-medium text-muted-foreground">
          Showing <span className="text-foreground">{students.length > 0 ? (page - 1) * limit + 1 : 0}</span> to{' '}
          <span className="text-foreground">{Math.min(page * limit, totalStudents)}</span> of{' '}
          <span className="text-foreground">{totalStudents}</span> entries
        </p>
      ...
      <StudentModals
        ...
        onDeleteConfirm={(id) => deleteMutation.mutate({ path: { student_id: id } })}
        isAddOpen={isAddOpen}
        ...
        onEditConfirm={(values) => studentToEdit && updateMutation.mutate({ path: { student_id: studentToEdit.id }, body: values })}
