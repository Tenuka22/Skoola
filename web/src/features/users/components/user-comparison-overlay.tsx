...
export function UserComparisonOverlay({
  ...
  const verifyRate = Math.round(
    (selectedData.filter((u) => u.is_verified).length / selectedUsers.size) * 100
  )

  const coreProvider = Object.entries(
  ...
  return (
    <div className="fixed bottom-10 left-1/2 z-50 flex -translate-x-1/2 items-center gap-8 rounded-3xl border border-primary/20 bg-background/90 p-7 shadow-[0_20px_50px_rgba(0,0,0,0.2)] backdrop-blur-2xl ring-1 ring-white/10 animate-in fade-in slide-in-from-bottom-12 duration-500">
      <div className="flex items-center gap-4 border-r border-border/50 pr-8">
        ...
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Cohort Analysis</p>
          <h4 className="text-base font-black tracking-tight">{selectedUsers.size} Users Selected</h4>
        </div>
      </div>

      <div className="flex gap-10">
        <div className="space-y-1.5">
          <span className="text-[11px] font-black text-muted-foreground/60 uppercase tracking-widest">Verify Rate</span>
          <p className="text-2xl font-black text-primary tabular-nums">{verifyRate}%</p>
        </div>
        <div className="space-y-1.5">
          <span className="text-[11px] font-black text-muted-foreground/60 uppercase tracking-widest">Core Provider</span>
