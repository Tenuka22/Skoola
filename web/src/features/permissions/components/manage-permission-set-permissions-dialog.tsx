...
import {
  assignPermissionToPermissionSet,
  unassignPermissionFromPermissionSet,
...
export function ManagePermissionSetPermissionsDialog({
  ...
  const { data: currentPermissionsInSet, isLoading: isLoadingCurrentPermissions } = useQuery({
    ...
    queryFn: async () => {
        ...
        return allPermissions.filter(p => p.id % 2 === 0); // Example: half of the permissions are in the set
    },
    enabled: open, // Only run when the dialog is open
  })

  React.useEffect(() => {
    if (currentPermissionsInSet) {
      setSelectedPermissions(new Set(currentPermissionsInSet.map(p => p.id)))
    }
...
const getPermissionCategory = (permissionName: string) => {
  ...
  if (permissionName.startsWith('PermissionSet')) return 'Permission Set Management'
  ...
  if (permissionName.startsWith('AcademicYear')) return 'Academic Year Permissions'
  ...
  if (permissionName.startsWith('ClassSubjectTeacher')) return 'Class Subject Teacher Permissions'
  ...
  if (permissionName.startsWith('ExamSubject')) return 'Exam Subject Permissions'
  if (permissionName.startsWith('Exam')) return 'Exam Permissions'
  if (permissionName.startsWith('GradingScheme')) return 'Grading Scheme Permissions'
  if (permissionName.startsWith('GradingCriterion')) return 'Grading Criterion Permissions'
