...
import { fetchPermissionSets, createPermissionSet, deletePermissionSet } from '../api'
import { RolePermissionsDialog } from './role-permissions-dialog'
import type { PermissionSet } from '../types'
import { HugeiconsIcon } from '@hugeicons/react'
import { Loading03Icon, UserGroupIcon, Add01Icon, HierarchyIcon } from '@hugeicons/core-free-icons'
...
export function RoleList() {
  const [selectedPermissionSet, setSelectedPermissionSet] = React.useState<PermissionSet | null>(null)
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p className="text-xs font-black uppercase tracking-widest opacity-50 italic">Initializing Security Mesh...</p>
      </div>
    )
  }
    ...
    <div className="space-y-12">
      <div className="flex items-center justify-between bg-muted/20 p-6 rounded-[2rem] ring-1 ring-border shadow-inner">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black tracking-tighter uppercase">Permission Set Management</h2>
            <p className="text-sm font-medium opacity-50">Define and manage granular access control policies.</p>
          </div>
        </div>
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="rounded-[2rem] border-none p-10 shadow-2xl backdrop-blur-3xl ring-1 ring-white/20">
            <DialogHeader>
              ...
              <DialogTitle className="text-center text-2xl font-black uppercase tracking-tight">Create Permission Set</DialogTitle>
              <DialogDescription className="text-center">
                Define a new set of permissions within the access control infrastructure.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-6 py-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest opacity-50">Permission Set Name</Label>
                <Input
                  value={newRoleName}
