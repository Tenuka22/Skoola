...
import { AlertCircleIcon, CalendarCheckIn01Icon, FloppyDiskIcon } from '@hugeicons/core-free-icons'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { format } from 'date-fns'
import * as React from 'react'
import { toast } from 'sonner'
import type { MarkStaffAttendanceRequest, StaffAttendanceResponse, StaffResponse } from '@/lib/api/types.gen'
...
export function StaffAttendanceDialog({
  ...
  const [status, setStatus] = React.useState<'Present' | 'Absent' | 'Late' | 'Excused' | 'HalfDay' | 'SchoolBusiness'>('Present')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl flex flex-col h-[85vh]">
        ...
        <div className="flex flex-col gap-6 flex-1 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4 p-4 border rounded-xl bg-muted/30">
            ...
            <div className="space-y-2">
              ...
              <Select value={status} onValueChange={(val: any) => setStatus(val)}>
                ...
                <SelectContent>
                  {['Present', 'Absent', 'Late', 'Excused', 'HalfDay', 'SchoolBusiness'].map((s) => (
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {isLoading ? (
              ...
              <p className="text-xs text-muted-foreground italic text-center p-8">No attendance records found.</p>
            ) : (
              <ScrollArea className="flex-1 border rounded-lg">
                <div className="divide-y">
                  {attendanceRecords.map((record: StaffAttendanceResponse, index: number) => (
                    <div key={index} className="p-3 flex items-center justify-between text-sm">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {record.time_in && `In: ${record.time_in}`} {record.time_out && `| Out: ${record.time_out}`}
                        </span>
                      </div>
                      <Badge
                        variant="secondary"
