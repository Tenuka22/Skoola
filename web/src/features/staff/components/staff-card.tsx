...
export function StaffCard({ staff, onEdit, onDelete }: StaffCardProps) {
  ...
  const isActive = staff.employment_status === 'Permanent' || staff.employment_status === 'Contract';
      ...
      <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48 rounded-2xl p-2">
            <DropdownMenuItem onClick={() => onEdit?.(staff)} className="rounded-xl h-10">
              <HugeiconsIcon icon={PencilEdit01Icon} className="w-4 h-4 mr-2" /> Edit Profile
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem 
            ...
            >
              <HugeiconsIcon icon={Delete02Icon} className="w-4 h-4 mr-2" /> Remove Staff
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      ...
      <CardContent className="flex flex-col items-center p-0">
        <div className="relative mb-6">
          ...
          <Avatar className="w-28 h-28 border-4 border-background ring-1 ring-border shadow-inner">
            <AvatarImage src={staff.photo_url || ''} alt={staff.name} className="object-cover" />
        ...
        <div className="grid grid-cols-1 w-full gap-3">
          <div className="flex items-center justify-between p-3 rounded-2xl bg-muted/30 group-hover:bg-muted/50 transition-colors">
            <div className="flex items-center text-sm font-medium text-muted-foreground">
              <HugeiconsIcon icon={HierarchyIcon} className="w-4 h-4 mr-2" /> Dept.
            </div>
            <span className="text-sm font-bold">{department}</span>
          </div>
          <div className="flex items-center justify-between p-3 rounded-2xl bg-muted/30 group-hover:bg-muted/50 transition-colors">
            <div className="flex items-center text-sm font-medium text-muted-foreground">
              <HugeiconsIcon icon={Calendar03Icon} className="w-4 h-4 mr-2" /> Hired
            </div>
        ...
        <div className="flex flex-col w-full mt-6 space-y-3">
          <div className="flex items-center px-4 py-3 rounded-2xl bg-background ring-1 ring-border shadow-sm group-hover:ring-primary/20 transition-all">
            ...
            <span className="text-xs font-bold truncate flex-1">{staff.email}</span>
          </div>
          <div className="flex items-center px-4 py-3 rounded-2xl bg-background ring-1 ring-border shadow-sm group-hover:ring-primary/20 transition-all">
            <div className="p-2 rounded-xl bg-muted/50 mr-3">
