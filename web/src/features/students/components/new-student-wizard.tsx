...
import { CreateStudentRequest, Gender, Ethnicity, Religion, StudentStatus } from '@/lib/api/types.gen';
...
export const NewStudentWizard = ({ onStudentAdded }: NewStudentWizardProps) => {
  ...
  const form = useForm<CreateStudentRequest>({
    resolver: zodResolver(zCreateStudentRequest),
    defaultValues: {
      name_english: '',
      admission_number: '',
      dob: new Date(),
      ...
      address: '',
      email: '',
      ethnicity: Ethnicity.Sinhala, // Default value
      religion: Religion.Buddhism, // Default value
      status: StudentStatus.Active, // Default value
    },
  });

  const createStudentMutation = useMutation({
    mutationFn: postStudents9Cfb76Aa83C6A83D99Db1D6755C24Ee1Mutation,
    onSuccess: () => {
  ...
  return (
    <Card className="w-full max-w-lg mx-auto">
      <CardHeader>
        <CardTitle>New Student Registration - Step {currentStep} of {totalSteps}</CardTitle>
        ...
        {currentStep === 1 && (
          <div className="grid gap-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="name_english" {...form.register("name_english")} className="col-span-3" />
              {form.formState.errors.name_english && <p className="col-span-4 text-right text-red-500 text-sm">{form.formState.errors.name_english.message}</p>}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="admission_number">Admission Number</Label>
              <Input id="admission_number" {...form.register("admission_number")} className="col-span-3" />
              {form.formState.errors.admission_number && <p className="col-span-4 text-right text-red-500 text-sm">{form.formState.errors.admission_number.message}</p>}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select onValueChange={(value: Gender) => form.setValue("gender", value)} defaultValue={form.watch("gender")}>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="nic_or_birth_certificate">NIC/Birth Certificate</Label>
              <Input id="nic_or_birth_certificate" {...form.register("nic_or_birth_certificate")} className="col-span-3" />
              {form.formState.errors.nic_or_birth_certificate && <p className="col-span-4 text-right text-red-500 text-sm">{form.formState.errors.nic_or_birth_certificate.message}</p>}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select onValueChange={(value: Ethnicity) => form.setValue("ethnicity", value)} defaultValue={form.watch("ethnicity")}>
              ...
              {form.formState.errors.ethnicity && <p className="col-span-4 text-right text-red-500 text-sm">{form.formState.errors.ethnicity.message}</p>}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="religion">Religion</Label>
              <Select onValueChange={(value: Religion) => form.setValue("religion", value)} defaultValue={form.watch("religion")}>
              ...
              {form.formState.errors.religion && <p className="col-span-4 text-right text-red-500 text-sm">{form.formState.errors.religion.message}</p>}
            </div>
          </div>
        )}
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button onClick={handleNext} disabled={createStudentMutation.isPending}>
          {currentStep < totalSteps ? "Next" : (createStudentMutation.isPending ? "Submitting..." : "Submit")}
        </Button>
      </CardFooter>
    </Card>
  );
