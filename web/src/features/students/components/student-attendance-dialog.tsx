...
import type { AttendanceStatus, StudentAttendanceResponse, StudentResponse } from '@/lib/api/types.gen'
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl flex flex-col h-[85vh]">
        <DialogHeader>
          <DialogTitle>Attendance Management: {student?.name_english}</DialogTitle>
          <DialogDescription>
        ...
        <div className="flex flex-col gap-6 flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {isLoading ? (
              ...
              <p className="text-xs text-muted-foreground italic text-center p-8">No attendance history found.</p>
            ) : (
              <ScrollArea className="flex-1 border rounded-lg">
                <div className="divide-y">
                  {attendanceRecords.map((record: StudentAttendanceResponse, index: number) => (
                    <div key={index} className="p-3 flex items-center justify-between text-sm">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">{record.remarks || 'No remarks'}</span>
                      </div>
                      <Badge
                        variant="secondary"
                        className={
