...
export const useStaffList = (params: { page?: number; limit?: number; search?: string } = {}) => {
  return useQuery({
    ...getStaffDb2Ddf96Bd86Cfcd0342B203Ba78A857Options({
      client: authClient,
      query: {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: [{ _id: 'getStaffAttendanceDateD0Fe5B3F1730787C38A30326Ac928B80' }],
      });
    },
    onError: (error) => {
...
export const useMarkStudentAttendanceBulk = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      toast.success('Attendance marked successfully');
      queryClient.invalidateQueries({
        queryKey: [{ _id: 'getStudentAttendanceClass2Fec35217B2F2C3727031Ce26765D12D' }],
      });
    },
    onError: (error) => {
      toast.error('Failed to mark attendance');
