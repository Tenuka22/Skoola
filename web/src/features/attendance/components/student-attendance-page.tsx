...
export const StudentAttendancePage = () => {
  ...
  const { data: studentsData, isLoading: isStudentsLoading } = useStudentsInClass(selectedClassId);
  const { data: attendanceData, isLoading: isAttendanceLoading, refetch: refetchAttendance } = useStudentAttendance(selectedClassId, formattedDateForApi);
  ...
  const filteredData = React.useMemo(() => {
    return mergedData.filter((item) =>
      item.student?.name_english.toLowerCase().includes(search.toLowerCase()) ||
      item.student?.admission_number.toLowerCase().includes(search.toLowerCase())
    );
  }, [mergedData, search]);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center gap-4">
         <div className="flex items-center gap-2 rounded-xl bg-background border p-1 shadow-sm">
            <HugeiconsIcon icon={Book01Icon} className="ml-2 size-4 text-muted-foreground" />
