...
export const useMultiAccount = () => {
  const [storedTokens, setStoredTokens] = useLocalStorage<Array<AuthToken>>(
    LOCAL_STORAGE_KEY,
    [],
  )
  const [activeAccountUserId, setActiveAccountUserId] = useLocalStorage<string>(
    ACTIVE_ACCOUNT_KEY,
    '',
  )

  const activeToken =
    storedTokens?.find((token) => token.userId === activeAccountUserId) || null
  ...
  const removeAccount = useCallback(
    (userId: string) => {
      const updatedTokens =
        storedTokens?.filter((token) => token.userId !== userId) || []
      setStoredTokens(updatedTokens)
  ...
  const switchAccount = useCallback(
    (userId: string) => {
      const accountExists = storedTokens?.some(
