...
export const FeeStructureManager = ({ /* props */ }: FeeStructureManagerProps) => {
  ...
  const addFeeRule = () => {
    if (newRule.grade && newRule.feeType && newRule.amount > 0) {
      setFeeRules(prev => [...prev, { ...newRule, id: Date.now().toString() }]);
        ...
        <CardContent>
          <div className="grid gap-4 mb-6">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="grade" value={newRule.grade} onChange={handleInputChange} className="col-span-3" placeholder="e.g., Grade 1" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="feeType">Fee Type</Label>
              <Input id="feeType" value={newRule.feeType} onChange={handleInputChange} className="col-span-3" placeholder="e.g., Tuition, Library" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="amount">Amount</Label>
              <Input id="amount" type="number" value={newRule.amount} onChange={handleInputChange} className="col-span-3" />
            </div>
            <Button onClick={addFeeRule} className="w-fit"><Icons.Plus className="h-4 w-4 mr-2" />Add Rule</Button>
          </div>
            ...
            <TableBody>
              {feeRules.map(rule => (
                <TableRow key={rule.id}>
                  ...
                  <TableCell className="text-right">{rule.amount.toFixed(2)}</TableCell>
                  <TableCell className="text-right">
                    <Button variant="destructive" size="icon" onClick={() => removeFeeRule(rule.id)}>
