...
use backend::models::resources::inventory::{UniformItem, UniformIssue};
use backend::models::auth::permission::{UserSet, UserSetUser, UserSetPermission, RolePermission, UserPermission};
...
impl SeedModule for SystemSeeder {
    fn seed(
        ...
        insert_into(school_calendar::table).values(&calendar_data).execute(conn)?;
        println!("Seeded {} calendar entries.", calendar_data.len());

        // 2. School Settings
        let settings_data = vec![
            SchoolSetting { setting_key: "school_name".to_string(), setting_value: "Skoola International".to_string(), description: Some("Main school name".to_string()), updated_at: Utc::now().naive_utc() },
            SchoolSetting { setting_key: "contact_email".to_string(), setting_value: "info@skoola.com".to_string(), description: Some("Main contact".to_string()), updated_at: Utc::now().naive_utc() },
        ];
        insert_into(school_settings::table).values(&settings_data).execute(conn)?;
        println!("Seeded {} school settings.", settings_data.len());

        // 3. Uniform Items & Issues
        let uniforms_data: Vec<UniformItem> = (0..seed_count_config.uniform_items).map(|i| {
            UniformItem { 
                id: generate_uuid(), 
                item_name: format!("{} Uniform Item {}", if i % 2 == 0 { "Boys" } else { "Girls" }, i + 1), 
                size: vec!["XS", "S", "M", "L", "XL"].choose(&mut rng).unwrap().to_string(), 
                gender: if i % 2 == 0 { "Male".to_string() } else { "Female".to_string() }, 
                grade_level: if rng.gen_bool(0.5) { Some(format!("Grade {}", rng.gen_range(1..=12))) } else { None }, 
        ...
        insert_into(uniform_items::table).values(&uniforms_data).execute(conn)?;
        println!("Seeded {} uniform items.", uniforms_data.len());
        
        if !context.student_ids.is_empty() && !context.staff_ids.is_empty() && !uniforms_data.is_empty() {
            let mut issues = Vec::new();
            for _ in 0..(context.student_ids.len().min(seed_count_config.uniform_issues_per_student * uniforms_data.len())) {
            ...
            insert_into(uniform_issues::table).values(&issues).execute(conn)?;
        ...
        insert_into(role_sets::table).values(&role_set).execute(conn)?;

        let role_set_role = RoleSetRole {
            role_set_id: role_set_id.clone(),
            role_id: RoleEnum::Teacher.to_string(),
        };
        insert_into(role_set_roles::table).values(&role_set_role).execute(conn)?;
        ...
        insert_into(user_sets::table).values(&user_set).execute(conn)?;
        ...
        if !context.user_ids.is_empty() {
             ...
             insert_into(user_set_users::table).values(&user_set_user).execute(conn)?;
        }
        ...
        insert_into(role_permissions::table).values(&role_perms).execute(conn)?;
        ...
        if !context.user_ids.is_empty() {
            ...
            insert_into(user_permissions::table).values(&user_perm).execute(conn)?;
        }
