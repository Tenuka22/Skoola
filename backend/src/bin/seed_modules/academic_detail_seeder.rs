...
use backend::models::academic::{GradeStream, GradeSubject, StreamSubject, Timetable, ClassSubjectTeacher};
    ...
    fn seed(
        ...
        if !context.grade_level_ids.is_empty() && !context.stream_ids.is_empty() {
            ...
            for grade_id in &context.grade_level_ids {
                ...
                for stream_id in context.stream_ids.iter().take(seed_count_config.grade_streams_per_grade) {
        ...
        if !context.grade_level_ids.is_empty() && !context.subject_ids.is_empty() {
            ...
            for grade_id in &context.grade_level_ids {
                ...
                for subject_id in shuffled_subjects.iter().take(seed_count_config.grade_subjects_per_grade) {
        ...
        if !context.stream_ids.is_empty() && !context.subject_ids.is_empty() {
            ...
            for stream_id in &context.stream_ids {
                ...
                use rand::seq::SliceRandom;
                shuffled_subjects.shuffle(&mut rng);
                for subject_id in shuffled_subjects.iter().take(seed_count_config.stream_subjects_per_stream) {
        ...
        if !context.student_ids.is_empty() && !context.class_ids.is_empty() && !context.academic_year_ids.is_empty() && !context.grade_level_ids.is_empty() {
            ...
            insert_into(student_class_assignments::table)
                ...
                .execute(conn)?;
            println!("Seeded {} student class assignments.", student_assignments.len());
        }

        // 5. Subject Enrollments
        if !context.student_ids.is_empty() && !context.subject_ids.is_empty() && !context.academic_year_ids.is_empty() {
            let academic_year_id = context.academic_year_ids[0].clone();
            ...
            for student_id in &context.student_ids {
                ...
                use rand::seq::SliceRandom;
                shuffled_subjects.shuffle(&mut rng);
                for subject_id in shuffled_subjects.iter().take(seed_count_config.subject_enrollments_per_student) {
            ...
            insert_into(subject_enrollments::table)
                ...
                .execute(conn)?;
            println!("Seeded {} subject enrollments.", subject_enrollments_data.len());
        }

        // 6. Teacher Class Assignments
        if !context.staff_ids.is_empty() && !context.class_ids.is_empty() && !context.academic_year_ids.is_empty() {
             let academic_year_id = context.academic_year_ids[0].clone();
             let mut teacher_class_assignments_data = Vec::new();
             for _ in 0..seed_count_config.teacher_class_assignments_per_class * context.class_ids.len() {
                let class_id = get_random_id(&context.class_ids);
             ...
             insert_into(teacher_class_assignments::table)
                 ...
                 .execute(conn)?;
             println!("Seeded {} teacher class assignments.", teacher_class_assignments_data.len());
        }

        // 7. Teacher Subject Assignments
        if !context.staff_ids.is_empty() && !context.subject_ids.is_empty() && !context.academic_year_ids.is_empty() {
            let academic_year_id = context.academic_year_ids[0].clone();
            ...
            for teacher_id in &context.staff_ids {
                ...
                use rand::seq::SliceRandom;
                shuffled_subjects.shuffle(&mut rng);
                for subject_id in shuffled_subjects.iter().take(seed_count_config.teacher_subject_assignments_per_teacher) {
            ...
            insert_into(teacher_subject_assignments::table)
                ...
                .execute(conn)?;
            println!("Seeded {} teacher subject assignments.", teacher_subject_assignments_data.len());
        }

        // 8. Class Subject Teachers
        if !context.class_ids.is_empty() && !context.subject_ids.is_empty() && !context.staff_ids.is_empty() && !context.academic_year_ids.is_empty() {
             let academic_year_id = context.academic_year_ids[0].clone();
             ...
             for class_id in &context.class_ids {
                 ...
                 use rand::seq::SliceRandom;
                 shuffled_subjects.shuffle(&mut rng);
                 for subject_id in shuffled_subjects.iter().take(seed_count_config.class_subject_teachers_per_class) {
             ...
             insert_into(class_subject_teachers::table)
                 .values(&cst_data)
                 .execute(conn)?;
             println!("Seeded {} class subject teacher assignments.", cst_data.len());
        }
                ...
                for day in &days {
                    for period in 1..=seed_count_config.timetable_entries_per_class_and_day {
                        ...
                        timetable_data.push(Timetable {
                            ...
                            teacher_id: get_random_id(&context.staff_ids),
                            start_time: chrono::NaiveTime::from_hms_opt(8 + period as u32, 0, 0).unwrap(),
                            end_time: chrono::NaiveTime::from_hms_opt(9 + period as u32, 0, 0).unwrap(),
