...
impl SeedModule for AttendanceSeeder {
    fn seed(
        ...
        insert_into(attendance_policies::table)
            ...
            .execute(conn)?;
        println!("Seeded {} attendance policies.", attendance_policies_data.len());

        // 2. Student Attendance
        if !context.student_ids.is_empty() && !context.class_ids.is_empty() && !context.user_ids.is_empty() {
            ...
            for i in 0..seed_count_config.student_attendance_entries {
                ...
                student_attendance_data.push(StudentAttendance {
                    ...
                    date,
                    status: if rng.gen_bool(0.9) { AttendanceStatus::Present } else { AttendanceStatus::Absent },
            ...
            insert_into(student_attendance::table)
                ...
                .execute(conn)?;
            println!("Seeded {} student attendance entries.", student_attendance_data.len());
        }

        // 3. Student Period Attendance
        if !context.student_ids.is_empty() && !context.class_ids.is_empty() && !context.timetable_ids.is_empty() && !context.user_ids.is_empty() {
             ...
             for i in 0..seed_count_config.student_period_attendance_entries {
                let date = NaiveDate::from_ymd_opt(2024, 2, (i as u32 % 28) + 1).unwrap();
                 period_attendance_data.push(StudentPeriodAttendance {
                     ...
                     date,
                     status: if rng.gen_bool(0.9) { AttendanceStatus::Present } else { AttendanceStatus::Late },
                     minutes_late: if rng.gen_bool(0.1) { Some(rng.gen_range(5..=30)) } else { Some(0) },
                     remarks: None,
                     is_locked: false,
                     marked_by: get_random_id(&context.user_ids),
                     created_at: Utc::now().naive_utc(),
             ...
             insert_into(student_period_attendance::table)
                 ...
                 .execute(conn)?;
             println!("Seeded {} student period attendance entries.", period_attendance_data.len());
        }
        ...
        if !context.staff_ids.is_empty() && !context.user_ids.is_empty() {
            ...
            insert_into(staff_attendance::table)
                ...
                .execute(conn)?;
            println!("Seeded {} staff attendance entries.", staff_attendance_data.len());
        }
        ...
        if !context.student_ids.is_empty() && !context.user_ids.is_empty() {
            ...
            for i in 0..seed_count_config.exit_passes {
                exit_passes_data.push(ExitPass {
                    ...
                    reason_type: vec![ExitReason::Medical, ExitReason::FamilyEvent, ExitReason::Other].choose(&mut rng).unwrap().clone(),
        ...
        if !context.student_ids.is_empty() && !context.user_ids.is_empty() {
            ...
            for i in 0..seed_count_config.pre_approved_absences {
                ...
                pre_approved_data.push(PreApprovedAbsence {
                    ...
                    start_date,
                    end_date,
                    reason_type: vec![PreApprovedReason::FamilyEvent, PreApprovedReason::Medical, PreApprovedReason::Other].choose(&mut rng).unwrap().clone(),
        ...
        if !context.user_ids.is_empty() {
            for _ in 0..seed_count_config.emergency_roll_calls {
                ...
                let roll_call = EmergencyRollCall {
                    ...
                    start_time: Utc::now().naive_utc() - chrono::Duration::minutes(rng.gen_range(10..=60)),
                ...
                for _ in 0..seed_count_config.emergency_roll_call_entries_per_roll_call {
                    entries.push(EmergencyRollCallEntry {
                        ...
                        status: vec![EmergencyStatus::Safe, EmergencyStatus::Missing, EmergencyStatus::Injured].choose(&mut rng).unwrap().clone(),
                ...
                insert_into(emergency_roll_call_entries::table)
                    .values(&entries)
                    .execute(conn)?;
                println!("Seeded 1 emergency roll call and {} entries.", entries.len());
            }
        }
        ...
        if !context.user_ids.is_empty() {
            ...
            for i in 0..seed_count_config.attendance_audit_logs {
                audit_logs.push(AttendanceAuditLog {
                    id: generate_uuid(),
                    attendance_type: if rng.gen_bool(0.5) { "Student".to_string() } else { "Staff".to_string() },
            ...
            insert_into(attendance_audit_log::table).values(&audit_logs).execute(conn)?;
