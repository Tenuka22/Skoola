...
use backend::database::enums::{PaymentMethod, FeeFrequency, TransactionType, ComponentType};
...
impl SeedModule for FinanceSeeder {
    fn seed(
        ...
        let chart_of_accounts_data: Vec<ChartOfAccount> = (0..seed_count_config.chart_of_accounts).map(|i| {
            let (code_prefix, name_prefix, type_prefix, balance_prefix) = base_chart_of_accounts[i % base_chart_of_accounts.len()];
            let account_code = format!("{}{:03}", code_prefix.chars().take(2).collect::<String>(), i + 1);
        ...
        let budget_categories_data = (0..seed_count_config.budget_categories).map(|i| {
            BudgetCategory {
                id: generate_uuid(),
        ...
        if context.academic_year_ids.is_empty() || context.budget_category_ids.is_empty() {
            ...
            let budgets_data = (0..seed_count_config.budgets).map(|i| {
                Budget {
                    id: generate_uuid(),
        ...
        // Seed Income Sources
        let income_sources_data = (0..seed_count_config.income_sources).map(|i| {
            IncomeSource {
                id: generate_uuid(),
        ...
        if context.income_source_ids.is_empty() || context.staff_ids.is_empty() {
            ...
            let income_transactions_data = (0..seed_count_config.income_transactions).map(|i| {
                IncomeTransaction {
                    id: generate_uuid(),
        ...
        let expense_categories_data = (0..seed_count_config.expense_categories).map(|i| {
            ExpenseCategory {
                id: generate_uuid(),
        ...
        if context.expense_category_ids.is_empty() || context.staff_ids.is_empty() {
            ...
            let expense_transactions_data = (0..seed_count_config.expense_transactions).map(|i| {
                ExpenseTransaction {
                    id: generate_uuid(),
                    ...
                    payment_method: vec![PaymentMethod::Cash, PaymentMethod::BankTransfer, PaymentMethod::Online].choose(&mut rng).unwrap().clone(),
                    approved_by: Some(get_random_id(&context.staff_ids)),
                    receipt_url: if rng.gen_bool(0.5) { Some(format!("http://example.com/receipts/{}.pdf", i + 1)) } else { None },
                    created_at: random_datetime_in_past(1),
                    updated_at: random_datetime_in_past(0),
                }
        ...
        // Seed Fee Categories
        let fee_categories_data = (0..seed_count_config.fee_categories).map(|i| {
            FeeCategory {
                id: generate_uuid(),
        ...
        if context.grade_level_ids.is_empty() || context.academic_year_ids.is_empty() || context.fee_category_ids.is_empty() {
            ...
            let fee_structures_data = (0..seed_count_config.fee_structures).map(|i| {
                FeeStructure {
                    id: generate_uuid(),
                    ...
                    frequency: vec![FeeFrequency::Monthly, FeeFrequency::Quarterly, FeeFrequency::Annually].choose(&mut rng).unwrap().clone(),
                    created_at: random_datetime_in_past(1),
                    updated_at: random_datetime_in_past(0),
                }
        ...
        if context.student_ids.is_empty() || context.fee_structure_ids.is_empty() {
            ...
            let student_fees_data = (0..seed_count_config.student_fees).map(|i| {
                StudentFee {
                    id: generate_uuid(),
                    ...
                    exemption_reason: if rng.gen_bool(0.1) { Some(format!("Financial Hardship {}", i + 1)) } else { None },
                    created_at: random_datetime_in_past(1),
                    updated_at: random_datetime_in_past(0),
                }
        ...
        if context.student_fee_ids.is_empty() || context.staff_ids.is_empty() {
            ...
            let fee_payments_data = (0..seed_count_config.fee_payments).map(|i| {
                FeePayment {
                    id: generate_uuid(),
                    ...
                    remarks: if rng.gen_bool(0.2) { Some(format!("Paid for month {}", (i % 12) + 1)) } else { None },
        ...
        let salary_components_data = (0..seed_count_config.salary_components).map(|i| {
            SalaryComponent {
                id: generate_uuid(),
                name: format!("Component {}", i + 1),
                component_type: vec![backend::database::enums::ComponentType::Allowance, backend::database::enums::ComponentType::Deduction, backend::database::enums::ComponentType::Bonus].choose(&mut rng).unwrap().clone(),
        ...
        if context.staff_ids.is_empty() || context.salary_component_ids.is_empty() {
            ...
            let staff_salaries_data = (0..seed_count_config.staff_salaries).filter_map(|_| {
        ...
        if context.staff_ids.is_empty() {
            ...
            let salary_payments_data = (0..seed_count_config.salary_payments).map(|i| {
                SalaryPayment {
                    id: generate_uuid(),
                    ...
                    payment_date: random_datetime_in_past(0),
                    payment_method: backend::database::enums::PaymentMethod::BankTransfer.to_string(),
                    remarks: if rng.gen_bool(0.2) { Some(format!("Bonus included for Q{}", (i % 4) + 1)) } else { None },
                    created_at: random_datetime_in_past(0),
                    updated_at: random_datetime_in_past(0),
                }
        ...
        if context.staff_ids.is_empty() {
            ...
            let petty_cash_transactions_data = (0..seed_count_config.petty_cash_transactions).map(|i| {
                PettyCashTransaction {
                    id: generate_uuid(),
                    amount: rng.gen_range(100.0..=1000.0),
                    transaction_type: vec![backend::database::enums::TransactionType::Received, backend::database::enums::TransactionType::Spent].choose(&mut rng).unwrap().clone(),
        ...
        if context.chart_of_account_ids.is_empty() {
            ...
            let general_ledger_data = (0..seed_count_config.general_ledger_entries).map(|i| {
