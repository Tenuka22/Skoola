...
use backend::models::resources::co_curricular::{
    Club, ClubActivity, ClubMember, Competition, CompetitionParticipant, CulturalEvent,
    CulturalEventParticipant, Sport, SportTeam, StudentAchievement,
};
use backend::models::system::activity::{
    Activity, ActivityAttendance, ActivityParticipant, ActivityType,
};
use backend::schema::*;
...
impl SeedModule for ExtracurricularSeeder {
    fn seed(
        ...
        // 1. Activity Types
        let activity_types_data: Vec<ActivityType> = (0..seed_count_config.activity_types)
            .map(|i| ActivityType {
                id: generate_uuid(),
                name: format!("Activity Type {}", i + 1),
                description: Some(format!("Educational activity type {}", i + 1)),
        ...
        // 2. Activities
        if !context.activity_type_ids.is_empty()
        ...
        if !context.activity_ids.is_empty() && !context.user_ids.is_empty() {
            ...
            for activity_id in &context.activity_ids {
                ...
                // Staff junction
                let staff_participants_count =
                    (seed_count_config.activity_participants_per_activity / 2).max(1);
                ...
                // Student junction
                let student_participants_count = seed_count_config
                    .activity_participants_per_activity
                    .saturating_sub(staff_participants_count);
            ...
            insert_into(activity_participants_staff::table)
                .values(&unique_staff_participants)
                .execute(conn)?;
            ...
            insert_into(activity_participants_students::table)
                .values(&unique_student_participants)
                .execute(conn)?;
        ...
        // 4. Sports
        let sports_data: Vec<Sport> = (0..seed_count_config.sports)
            .map(|i| Sport {
                id: generate_uuid(),
                sport_name: format!("Sport {}", i + 1),
                description: Some(format!("Description for Sport {}", i + 1)),
        ...
        // 5. Clubs
        if !context.staff_ids.is_empty() {
            let clubs_data: Vec<Club> = (0..seed_count_config.clubs)
                .map(|i| Club {
                    id: generate_uuid(),
                    club_name: format!("Club {}", i + 1),
                    description: Some(format!("Exploring club {}", i + 1)),
                    teacher_in_charge_id: get_random_id(&context.staff_ids),
                    meeting_schedule: Some(format!(
                        "{}s 2pm",
                ...
                for i in 0..seed_count_config.club_activities_per_club {
                    club_acts.push(ClubActivity {
                        ...
                        description: Some(format!(
                            "Discussion of goals for club activity {}",
                            i + 1
                        )),
            ...
            insert_into(club_activities::table)
                .values(&club_acts)
                .execute(conn)?;
        ...
        // 7. Competitions
        let competitions_data: Vec<Competition> = (0..seed_count_config.competitions)
            .map(|i| Competition {
                id: generate_uuid(),
                competition_name: format!("Competition {}", i + 1),
                competition_type: if i % 2 == 0 {
                    "Sports".to_string()
                } else {
                    "Academic".to_string()
                },
                date: Utc::now().naive_utc() - chrono::Duration::days(rng.gen_range(0..365)),
                organizer: format!("Organizer {}", i + 1),
                level: if i % 3 == 0 {
                    "Provincial".to_string()
                } else {
                    "National".to_string()
                },
                created_at: Utc::now().naive_utc(),
        ...
        // 8. Cultural Events
        let cultural_events_data: Vec<CulturalEvent> = (0..seed_count_config.cultural_events)
            .map(|i| CulturalEvent {
                id: generate_uuid(),
                event_name: format!("Cultural Event {}", i + 1),
                event_date: Utc::now().naive_utc() - chrono::Duration::days(rng.gen_range(0..365)),
                venue: format!("Venue {}", i + 1),
                description: Some(format!("Description for cultural event {}", i + 1)),
                created_at: Utc::now().naive_utc(),
                updated_at: Utc::now().naive_utc(),
            })
            .collect();
        insert_into(cultural_events::table)
            .values(&cultural_events_data)
            .execute(conn)?;
        context.cultural_event_ids = cultural_events_data.into_iter().map(|e| e.id).collect();
        println!(
            "Seeded {} cultural events.",
            context.cultural_event_ids.len()
        );
        ...
        if !context.student_ids.is_empty() {
            ...
            if !context.club_ids.is_empty() {
                ...
                insert_into(club_members::table)
                    .values(&members)
                    .execute(conn)?;
            ...
            if !context.competition_ids.is_empty() {
                ...
                for competition_id in &context.competition_ids {
                    for _ in 0..seed_count_config.competition_participants_per_competition {
                        let student_id = get_random_id(&context.student_ids);
                        if seen_comp_participants
                            .insert((competition_id.clone(), student_id.clone()))
                        {
                ...
                insert_into(competition_participants::table)
                    .values(&comp_participants)
                    .execute(conn)?;
                println!(
                    "Seeded {} competition participants.",
                    comp_participants.len()
                );
            }
            ...
            if !context.cultural_event_ids.is_empty() {
                ...
                insert_into(cultural_event_participants::table)
                    .values(&cultural_event_participants_data)
                    .execute(conn)?;
                println!(
                    "Seeded {} cultural event participants.",
                    cultural_event_participants_data.len()
                );
            }
            ...
            insert_into(student_achievements::table)
                .values(&achievements)
                .execute(conn)?;
