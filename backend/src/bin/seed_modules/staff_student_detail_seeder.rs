...
use backend::models::student::history::StudentPreviousSchool;
...
impl SeedModule for StaffStudentDetailSeeder {
    fn seed(
        ...
        // 1. Staff Departments
        let departments_data: Vec<StaffDepartment> = (0..seed_count_config.staff_departments)
            .map(|i| StaffDepartment {
                id: generate_uuid(),
                name: format!("Department {}", i + 1),
                description: Some(format!("Department of {}", i + 1)),
                created_at: Utc::now().naive_utc(),
                updated_at: Utc::now().naive_utc(),
            })
            .collect();
        insert_into(staff_departments::table)
        ...
        if !context.staff_ids.is_empty() {
            ...
            for _ in 0..(context.staff_ids.len() * seed_count_config.staff_qualifications_per_staff)
            {
                qualifications.push(StaffQualification {
                    ...
                    degree: format!(
                        "{} of Education",
                        vec!["Bachelor", "Master", "PhD"].choose(&mut rng).unwrap()
                    ),
                    institution: format!(
                        "University of {}",
                        vec!["Colombo", "Peradeniya", "Moratuwa", "Jaffna"]
                            .choose(&mut rng)
                            .unwrap()
                    ),
        ...
        if !context.staff_ids.is_empty() {
            ...
            for _ in
                0..(context.staff_ids.len() * seed_count_config.staff_employment_history_per_staff)
            {
                ...
                let end_date = if rng.gen_bool(0.7) {
                    Some(start_date + chrono::Duration::days(rng.gen_range(365..1825)))
                } else {
                    None
                }; // 1 to 5 years later
                history.push(StaffEmploymentHistory {
                    ...
                    position: format!(
                        "{} Teacher",
                        vec!["Assistant", "Senior", "Head"]
                            .choose(&mut rng)
                            .unwrap()
                    ),
                    start_date,
                    end_date,
                    reason_for_leaving: if rng.gen_bool(0.5) {
                        Some("Better opportunity".to_string())
                    } else {
                        None
                    },
                    created_at: Utc::now().naive_utc(),
        ...
        if !context.staff_ids.is_empty() {
            ...
            for _ in 0..(context.staff_ids.len() * seed_count_config.staff_leaves_per_staff) {
                ...
                leaves.push(StaffLeave {
                    ...
                    leave_type: format!(
                        "{} Leave",
                        vec!["Sick", "Casual", "Maternity", "Study"]
                            .choose(&mut rng)
                            .unwrap()
                    ),
                    from_date,
        ...
        if !context.student_ids.is_empty() {
            ...
            for _ in
                0..(context.student_ids.len() * seed_count_config.student_medical_info_per_student)
            {
                medical_info.push(StudentMedicalInfo {
                    ...
                    allergies: if rng.gen_bool(0.3) {
                        Some(format!(
                            "{}",
                            vec!["Peanuts", "Dust", "Pollen", "Penicillin"]
                                .choose(&mut rng)
                                .unwrap()
                        ))
                    } else {
                    ...
                    medical_conditions: if rng.gen_bool(0.2) {
                        Some(format!(
                            "{}",
                            vec!["Asthma", "Diabetes", "Epilepsy"]
                                .choose(&mut rng)
                                .unwrap()
                        ))
                    } else {
        ...
        if !context.student_ids.is_empty() {
            ...
            for _ in 0..(context.student_ids.len()
                * seed_count_config.student_emergency_contacts_per_student)
            {
                contacts.push(StudentEmergencyContact {
                    ...
                    relationship: format!(
                        "{}",
                        vec!["Father", "Mother", "Guardian", "Sibling"]
                            .choose(&mut rng)
                            .unwrap()
                    ),
        ...
        if !context.student_ids.is_empty() {
            ...
            for _ in 0..(context.student_ids.len()
                * seed_count_config.student_previous_schools_per_student)
            {
                let date_left = Some(
                    NaiveDate::from_ymd_opt(
                        2018 + rng.gen_range(0..5),
                        rng.gen_range(1..=12),
                        rng.gen_range(1..=28),
                    )
                    .unwrap(),
                ...
                prev_schools.push(StudentPreviousSchool {
                    ...
                    date_left,
                    reason_for_leaving: if rng.gen_bool(0.6) {
                        Some("Relocation".to_string())
                    } else {
                        None
                    },
                    created_at: Utc::now().naive_utc(),
            ...
            insert_into(student_previous_schools::table)
                .values(&prev_schools)
                .execute(conn)?;
