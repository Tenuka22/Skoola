...
pub async fn send_verification_email(config: &Config, recipient_email: &str, token: &str) -> Result<bool, APIError> {
    ...
    let verification_link = format!(
        "{}/{}",
        config.email_verification_base_url, token
    );
    ...
    let sender = config.smtp_sender_email.clone().ok_or_else(|| APIError::internal("Sender email not configured"))?;
    ...
    match mailer.send(&email) {
        ...
        Err(e) => {
            Err(APIError::internal(&format!("Failed to send verification email: {:?}", e)))
        }
    }
}

/// Sends a password reset email to a recipient with a given token.
pub async fn send_password_reset_email(config: &Config, recipient_email: &str, token: &str) -> Result<bool, APIError> {
    if !config.send_emails {
    ...
    let reset_link = format!(
        "{}/{}",
        config.password_reset_base_url, token
    );
...
fn create_mailer(config: &Config) -> Result<SmtpTransport, APIError> {
    let host = config.smtp_host.as_deref().ok_or_else(|| APIError::internal("SMTP host not configured"))?;
