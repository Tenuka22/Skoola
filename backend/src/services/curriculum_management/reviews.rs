...
pub async fn submit_review(
    ...
    let id = generate_prefixed_id(&mut conn, IdPrefix::REVIEW)?;
    ...
    for student in students {
        ...
        for guardian in guardians {
            if let Some(g_email) = guardian.email {
                let _ = send_email(&pool.config, g_email, format!("Lesson Summary for {}: {}", student.name_english, progress.lesson_summary), body.clone()).await;
            }
        }
    }
