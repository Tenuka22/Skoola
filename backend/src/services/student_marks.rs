...
use crate::{
    ...
    models::{
        ...
        student_class_assignment::StudentClassAssignment,
        student::Student,
        exam_subjects::ExamSubject, // Added ExamSubject
    },
};
use actix_web::{web, HttpResponse};
use uuid::Uuid;
use chrono::Utc;
use crate::schema::{student_marks, student_class_assignments, students, exam_subjects}; // Added exam_subjects

// Service to create a new StudentMark
pub async fn create_student_mark(
    pool: web::Data<AppState>,
    new_student_mark_request: CreateStudentMarkRequest,
    current_user_id: String, // Added this
    ...
    let exam_subject: ExamSubject = exam_subjects::table
        ...
        .first(&mut conn)
        .map_err(|e| match e {
            diesel::result::Error::NotFound => APIError::bad_request(&format!("ExamSubject with Exam ID {} and Subject ID {} not found", new_student_mark_request.exam_id, new_student_mark_request.subject_id)),
    ...
    let new_student_mark = StudentMark {
        ...
        entered_by: current_user_id.clone(), // Set from current_user_id
...
pub async fn get_student_mark_by_id(
    ...
    let student_mark: StudentMark = student_marks::table
        ...
        .first(&mut conn)
        .map_err(|e| match e {
            diesel::result::Error::NotFound => APIError::not_found(&format!("Student Mark with ID {} not found", student_mark_id)),
            _ => APIError::internal(&e.to_string()),
        })?;
...
pub async fn update_student_mark(
    ...
    if let Some(marks_obtained) = update_request.marks_obtained {
        ...
        let exam_subject: ExamSubject = exam_subjects::table
            ...
            .first(&mut conn)
            .map_err(|e| match e {
                diesel::result::Error::NotFound => APIError::bad_request(&format!("ExamSubject for Exam ID {} and Subject ID {} not found", existing_student_mark.exam_id, existing_student_mark.subject_id)),
                _ => APIError::internal(&e.to_string()),
            })?;
    ...
    let updated_count = diesel::update(target)
        .set((
            update_request,
            student_marks::updated_by.eq(current_user_id), // Set from current_user_id
    ...
    conn.transaction(|conn| {
        for req in bulk_request.marks {
            ...
            let exam_subject: ExamSubject = exam_subjects::table
                ...
                .first(conn)
                .map_err(|e| match e {
                    diesel::result::Error::NotFound => APIError::bad_request(&format!("ExamSubject with Exam ID {} and Subject ID {} not found", req.exam_id, req.subject_id)),
                    _ => APIError::internal(&e.to_string()),
                })?;

            // Validate marks_obtained
            if req.marks_obtained < 0 || req.marks_obtained > exam_subject.max_marks {
                return Err(diesel::result::Error::RollbackTransaction.into()); // Rollback on validation error
            }
        ...
        // Handle the error, potentially transforming it into an APIError
        APIError::internal(&format!("Failed to bulk create student marks: {}", e))
    })?;
