...
pub fn configure(cfg: &mut web::ServiceConfig) {
    ...
    cfg.service(
        web::scope("/library")
            ...
            .route(
                "/categories/bulk",
                web::delete().to(library::bulk_delete_library_categories),
            )
            .route(
                "/categories/bulk",
                web::patch().to(library::bulk_update_library_categories),
            )
            .route("/books", web::get().to(library::get_all_books))
            .route(
                "/books/bulk",
                web::delete().to(library::bulk_delete_library_books),
            )
            .route(
                "/books/bulk",
                web::patch().to(library::bulk_update_library_books),
            )
            ...
            .route(
                ...
                web::get().to(library::get_books_by_category),
            )
            .route("/issues", web::post().to(library::issue_book))
            .route(
                "/issues/{issue_id}",
                web::get().to(library::get_issue_by_id),
            )
            .route(
                "/issues/{issue_id}/return",
                web::post().to(library::return_book),
            )
            .route(
                "/issues/student/{student_id}",
                web::get().to(library::get_issued_books_by_student),
            )
            .route(
                "/issues/staff/{staff_id}",
                web::get().to(library::get_issued_books_by_staff),
            )
            ...
            .route(
                ...
                web::post().to(library::waive_fine),
            )
