use apistos::web;
use crate::handlers::exams::{exam_subjects, exam_types, exams, grading_criteria, grading_schemes, report_cards, special_exams, zscore};
...
pub fn configure(cfg: &mut web::ServiceConfig) {
    ...
    .service(
        web::scope("/exams")
            ...
            .route("/term/{term_id}", web::get().to(exams::get_exams_by_term_id))
    ...
    .service(
        web::scope("/exam-subjects")
            ...
            .route("/schedule/academic-year/{academic_year_id}/term/{term_id}", web::get().to(exam_subjects::get_exam_schedule))
    ...
    .service(
        web::scope("/grading-schemes")
            .wrap(PermissionVerification { required_permission: PermissionEnum::GradingSchemeManage })
            .wrap(Authenticated)
            .route("", web::post().to(grading_schemes::create_grading_scheme_handler))
            .route("", web::get().to(grading_schemes::get_all_grading_schemes_handler))
            .route("/{id}", web::get().to(grading_schemes::get_grading_scheme_by_id_handler))
            .route("/{id}", web::put().to(grading_schemes::update_grading_scheme_handler))
            .route("/{id}", web::delete().to(grading_schemes::delete_grading_scheme_handler))
            .route("/{scheme_id}/assign_grade_level/{grade_level_id}", web::put().to(grading_schemes::assign_grading_scheme_to_grade_level_handler)),
    )
    .service(
        web::scope("/grading-criteria")
            .wrap(PermissionVerification { required_permission: PermissionEnum::GradingCriterionManage })
            .wrap(Authenticated)
            .route("", web::post().to(grading_criteria::create_grading_criterion_handler))
            .route("/{id}", web::get().to(grading_criteria::get_grading_criterion_by_id_handler))
            .route("/{id}", web::put().to(grading_criteria::update_grading_criterion_handler))
            .route("/{id}", web::delete().to(grading_criteria::delete_grading_criterion_handler)),
    )
    .service(
