...
use crate::{
    AppState,
    errors::APIError,
    models::student::{CreateStudentRequest, UpdateStudentRequest, PaginationInfo, StudentSearchQuery, StudentFilterQuery, StudentResponse, Student},
    services::student,
};
...
pub async fn upload_student_photo(
    ...
    let _student_member: Student = students::table
        ...
        .first(&mut conn)
        .map_err(|e| match e {
            diesel::result::Error::NotFound => APIError::not_found(&format!("Student with ID {} not found", student_id_inner)),
    ...
    if let Some(filepath) = file_path {
        ...
        Ok(HttpResponse::Ok().json(StudentResponse::from(updated_student)))
    } else {
