...
use crate::{
    ...
    models::student::attendance::{
        AttendanceExcuseResponse, BulkMarkStudentAttendanceRequest, ExitPassResponse,
        GenerateAttendanceReportRequest, GetAttendanceByClassAndDatePath,
        GetAttendanceByStudentPath, InitiateEmergencyRollCallRequest, IssueExitPassRequest,
        LowAttendanceStudentQuery, MarkPeriodAttendanceRequest, MarkStudentAttendanceRequest,
        SendAbsenceNotificationRequest, StudentAttendanceReportResponse, StudentAttendanceResponse,
...
pub async fn bulk_mark_student_attendance(
    ...
    let marked_records =
        student_attendance::bulk_mark_student_attendance(data.clone(), body.into_inner()).await?;
...
pub async fn mark_individual_student_attendance(
    ...
    let marked_record =
        student_attendance::mark_individual_student_attendance(data.clone(), body.into_inner())
            .await?;
...
pub async fn update_student_attendance(
    ...
    let updated_record = student_attendance::update_student_attendance(
        data.clone(),
        attendance_id,
        body.into_inner(),
    )
...
pub async fn get_attendance_by_class_and_date(
    ...
    let records = student_attendance::get_attendance_by_class_and_date(
        data.clone(),
        path_params.class_id,
        path_params.date,
    )
    .await?;
    Ok(Json(records))
...
pub async fn get_students_with_low_attendance(
    ...
    let students =
        student_attendance::get_students_with_low_attendance(data.clone(), query).await?;
    Ok(Json(students))
}
...
pub async fn send_absence_notifications(
    ...
    student_attendance::send_absence_notifications(data.clone(), body.class_id.clone(), body.date)
        .await?;
    Ok(Json(MessageResponse {
        message: "Absence notifications process initiated.".to_string(),
    }))
}
...
pub async fn initiate_emergency_roll_call(
    ...
    let roll_call_id =
        student_attendance::initiate_emergency_roll_call(data, body.event_name.clone(), user_id.0)
            .await?;
...
pub async fn update_emergency_status(
    ...
    student_attendance::update_emergency_entry(
        data,
        rc_id,
        u_id,
        body.status.clone(),
...
pub async fn complete_emergency_roll_call(
    ...
    Ok(Json(MessageResponse {
        message: "Roll call completed".to_string(),
    }))
}
...
pub async fn sync_pre_approved_absences(
    ...
    let count: i32 =
        student_attendance::apply_pre_approved_absences(data, path.into_inner().date).await?;
    Ok(Json(count))
}
...
pub async fn run_discrepancy_check(
    ...
    let count: i32 =
        student_attendance::run_discrepancy_check(data, path.into_inner().date).await?;
    Ok(Json(count))
}
...
pub async fn get_enriched_student_list(
    ...
    Json<Vec<crate::services::students::student_attendance::EnrichedStudentAttendance>>,
    APIError,
> {
    let path_inner = path.into_inner();
    let res: Vec<student_attendance::EnrichedStudentAttendance> =
        student_attendance::get_enriched_student_list(data, path_inner.class_id, path_inner.date)
            .await?;
    Ok(Json(res))
}
#[api_operation(
...
pub async fn mark_period_attendance(
    ...
    Ok(Json(MessageResponse {
        message: "Period attendance marked successfully.".to_string(),
    }))
}
...
pub async fn issue_exit_pass(
    ...
    let user_id = UserId::from_request(&req)?;
    let res = attendance_policies::issue_exit_pass(
        data,
